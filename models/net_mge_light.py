...
class Network(M.Module):
    ...
    def __init__(self):
        ...
        self.conv0 = Conv2D(in_channels=4, out_channels=8, kernel_size=3, padding=1, stride=1, is_seperable=False, has_relu=True)
        self.enc1 = EncoderStage(in_channels=8, out_channels=32, num_blocks=2)
        ...
        self.encdec = Conv2D(in_channels=256, out_channels=32, kernel_size=3, padding=1, stride=1, is_seperable=True, has_relu=True)
        self.dec1 = DecoderStage(in_channels=32, skip_in_channels=128, out_channels=32)
        self.dec2 = DecoderStage(in_channels=32, skip_in_channels=64, out_channels=16)
        self.dec3 = DecoderStage(in_channels=16, skip_in_channels=32, out_channels=16)
        self.dec4 = DecoderStage(in_channels=16, skip_in_channels=8, out_channels=8)

        self.out0 = DecoderBlock(in_channels=8, out_channels=8, kernel_size=3)
        self.out1 = Conv2D(in_channels=8, out_channels=4, kernel_size=3, stride=1, padding=1, is_seperable=False, has_relu=False)
